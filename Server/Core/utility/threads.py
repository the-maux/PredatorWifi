...
from Plugins.FrontServer.FrontServer import ServerFront
...
class ThreadFrontServer(QThread):
    ...
    def launchApiProbe(self):
        self.main.btn_probe.trigger()
        self.main.Fprobe.StartProbeResquest()

    def stopApiProbe(self):
        print "Stop Probe"
        self.main.Fprobe.StopProbeResquest()
        self.main.Fprobe.close()

    def restartApServer(self):
        print "RESTARTING AP EVIL"
        self.main.btn_cancelar.click()
        QtCore.QTimer.singleShot(4000, lambda: self.main.btn_start_attack.click())

    def remoteControl(self):
        print "Entering Remote Control\t\t\t[+]"
        if "/start" or "/stop" or "/restart" in self.server.Message:
            switcher = {
                "/start ApServer": self.main.btn_start_attack.click,
                "/stop ApServer": self.main.btn_cancelar.click,
                "/restart ApServer": self.restartApServer,
                "/start ProbeMonitor": self.launchApiProbe,
                "/stop ProbeMonitor": self.stopApiProbe,
            }
            switcher.get(self.server.Message, self.nothing)()
        if "/ApName" in self.server.Message:
            self.main.EditApName.setText(self.server.Message[len("/ApName "):])
            print "APNAME MAIN:" + self.main.EditApName.text() + " For :" + self.server.Message[len("/ApName "):]
        elif "/Channel" in self.server.Message:
            self.main.EditChannel.setText(self.server.Message[len("/Channel "):])

    def run(self):
        print 'Mon Starting Thread:' + self.objectName()
        self.emit(QtCore.SIGNAL("launch front"))
