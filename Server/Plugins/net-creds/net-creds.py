#!/usr/bin/env python2
#credits: DanMcInerney
#git: https://github.com/DanMcInerney/net-creds/blob/master/net-creds.py
from os import geteuid, devnull,chdir
import logging
# shut up scapy
...
def printer(src_ip_port, dst_ip_port, msg):
    ...
    if dst_ip_port != None:
        ...
        for s in skip:
            if s not in msg:
                if os.path.isfile('Logs/credentials.txt'):
                    with open('Logs/credentials.txt', 'r') as log:
                        contents = log.read()
                        if msg in contents:
                            return
